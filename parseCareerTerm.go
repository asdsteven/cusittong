...
func (s *parser) parseCareerTerm(icsid, careers, terms string) error {
...
setFocus_win0('CLASS_SRCH_WRK2_STRM$50$',-1);
...
`
	...
	if err := s.spanErr(careers); err != nil {
		return fmt.Errorf("careers\n%v", err)
	}
	if err := s.spanErr(afterCareers); err != nil {
		return fmt.Errorf("afterCareers\n%v", err)
	}
